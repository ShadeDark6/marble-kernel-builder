...
| Kernel source (`aospa-shadedark`) | `aospa-shadedark/android_kernel_xiaomi_sm8450` | `calcite` | LOS-family GKI fragments; single-repo build only (companion -modules/-devicetrees repos not cloned) |
...
- **LOS-family** presets (`lineageos`, `evolution-x`, `aosp-pablo`, `pa-gr`, `aospa-shadedark`) should use `toolchain=llvm-22.1.8` and `lto=thin` on free runners; the workflow enables swap and caps parallelism to reduce OOM risk. `lto=full` is experimental on free runners (OOM risk).
